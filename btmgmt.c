...
#define START_ADV 1     /*start adv*/
#define ADD_ADV_DATA 0  /*start adv ,set adv data,set scan rsp data*/
...
static char adapter_addr[18];
static int flag = 0;
static int pending_index = 0;
static char* argv_adv[]= {"","-d","0b09574c414e436f6e666967","-D","60","1"};
#ifndef WAIT_ANY
#define WAIT_ANY (-1)
#endif
...
static struct queue *submenus;

static void cmd_advertising(int argc, char **argv);

static void bt_shell_set_prompt(char *str)
{
}

static void bt_shell_noninteractive_quit(int status)
{
}

static void bt_shell_prompt_input(const char *label, const char *msg,
			bt_shell_prompt_input_func func, void *user_data)
{
}

static void bt_shell_usage(void)
{
}

static bool bt_shell_add_submenu(const struct bt_shell_menu *menu)
{
	if (!submenus) {
		submenus = queue_new();
		if (!submenus)
	...
	queue_push_tail(submenus, (void *) menu);
							...
							void *user_data)
{
	...
	#if ADD_ADV_DATA
	cmd_add_adv(6,argv_adv);
	#endif

	#if START_ADV
	if (send_cmd(mgmt, MGMT_OP_SET_ADVERTISING, index, sizeof(val), &val, setting_rsp) == 0) {
		error("Unable to send %s cmd", mgmt_opstr(MGMT_OP_SET_ADVERTISING));
							...
							void *user_data)
{
	...
	ba2str(&rp->bdaddr, adapter_addr);
	flag = 1;
	print("\taddr %s version %u manufacturer %u class 0x%02x%02x%02x",
			adapter_addr, rp->version, le16_to_cpu(rp->manufacturer),
...
#define  g_slist_next(slist)  ((slist) ? (((LTKS *)(slist))->next) : NULL)
struct smp_ltk_info {
	bdaddr_t bdaddr;
	uint8_t bdaddr_type;
...
static struct smp_ltk_info *get_ltk(FILE *file,const char *peer)
{
	struct smp_ltk_info *ltk = NULL;
	char key[33];
	char *rand = NULL;

	ltk = malloc(sizeof(struct smp_ltk_info));
	...
	if(fgets(line,sizeof(line),file)!=NULL){
		// print("peer_type=%s",line);
		ltk->bdaddr_type=(uint8_t)(atoi(line));
		// printf("%d\n",ltk->bdaddr_type );
	}
	...
	str2buf(key, ltk->val, sizeof(ltk->val));
	// print("ltk->val=%ld",strlen(ltk->val));

	if(fgets(line,sizeof(line),file)!=NULL){
		// print("Rand=%s",line);
		ltk->rand=(uint64_t)(atoi(line));
		// printf("%ld\n",ltk->rand);
	}
	// if (!rand)
	// 	goto failed;
	...
	if(fgets(line,sizeof(line),file)!=NULL){
		// print("Authenticated=%s",line);
		ltk->authenticated=(uint8_t)(atoi(line));
	}

	if(fgets(line,sizeof(line),file)!=NULL){
		// print("enc_size=%s",line);
		ltk->enc_size=(uint8_t)(atoi(line));
	}

	if(fgets(line,sizeof(line),file)!=NULL){
		print("EDiv=%s",line);
		ltk->ediv=(uint16_t)(atoi(line));
	}
...
LTKS * CreateListTail(LTKS *head,void* data){
	LTKS* newNode = (LTKS*)malloc(sizeof(LTKS));
	if(newNode == NULL){
		error("create node failed");
		exit(1);
	}
	newNode->data = data;
	newNode->next = NULL;
...
static void load_ltks( LTKS *keys,uint16_t index)
{
	...
	key_count = traverseList(keys);

	print("key count: %zu",  key_count);
	...
	cp = malloc(sizeof(struct mgmt_cp_load_long_term_keys)*cp_size);
	if (cp == NULL) {
		error("No memory for LTKs for hci0");
		return;
	}
	...
	for (l = keys, key = cp->keys; l && key_count;
		...
		print("key->val=%s",key->val);
		key->rand = cpu_to_le64(info->rand);
		print("key->rand=%ld",key->rand);
		key->ediv = cpu_to_le16(info->ediv);
		print("key->ediv=%d",key->ediv);
		key->type = info->authenticated;
		print("key->type=%d",key->type);
		key->central = info->central;
		print("key->central=%d",key->central);
		key->enc_size = info->enc_size;
		print("key->enc_size=%d",key->enc_size);
	 ...
	 */
	if (mgmt_send(mgmt, MGMT_OP_LOAD_LONG_TERM_KEYS, index, cp_size, cp,
...
static void load_devices(uint16_t index)
{
	...
	print("dirname name = %s",dirname);
	dir = opendir(dirname);
	if (!dir) {
		error("Unable to open adapter storage directory: %s",
								dirname);
		return;
	}
	...
	while ((entry = readdir(dir)) != NULL) {
		...
		create_filename(filename, PATH_MAX, "/%s/%s/LTK",
					adapter_addr,
					entry->d_name);
		...
		ltk_info = get_ltk(file, entry->d_name);

		// If any key for the device is blocked, we discard all.
		if (ltk_info && ltk_info->is_blocked ) {
		...
		if (ltk_info)
			ltks = CreateListTail(ltks, ltk_info);
		
	}
	closedir(dir);
	load_ltks(ltks,index);
	free(ltks);
}

static void load_device_rsp(uint8_t status, uint16_t len, const void *param,
							void *user_data)
{
	const struct mgmt_rp_read_info *rp = param;
	uint16_t index = PTR_TO_UINT(user_data);
...
static void cmd_ltks(int argc, char **argv)
{
	...
	if (index == MGMT_INDEX_NONE)
		index = 0;
	if (!mgmt_send(mgmt, MGMT_OP_READ_INFO, mgmt_index, 0, NULL, load_device_rsp,
					UINT_TO_PTR(mgmt_index), NULL)) {
		error("Unable to send read_info cmd");
		return bt_shell_noninteractive_quit(EXIT_FAILURE);
	}
...
static void store_ltk_group(const bdaddr_t *peer,
				uint8_t bdaddr_type, const unsigned char *key, uint8_t authenticated,
				uint8_t enc_size, uint16_t ediv,uint64_t rand)
{
	char device_addr[18];
	char filename[PATH_MAX];
	char key_str[33];
	...
	fprintf(file,"%s\n",key_str);
	print("adde_type=%d",bdaddr_type);
	fprintf(file,"%d\n",bdaddr_type);
	print("Rand=%ld",rand);
	fprintf(file,"%ld\n",rand);
	print("Authenticated=%d",authenticated);
	fprintf(file,"%d\n",authenticated);
	print("EncSize=%d",enc_size);
	fprintf(file,"%d\n",enc_size);
	print("EDiv=%d",ediv);
	fprintf(file,"%d\n",ediv);
	
	
	// if(fwrite(key_str,sizeof(char),strlen(key_str),file) != strlen(key_str)){
	// 	perror("write failed");
	// 	exit(1);
	// }
	fclose(file);
...
static void store_longtermkey(const bdaddr_t *peer,
				...
				uint64_t rand)
{
	...
	store_ltk_group(peer, bdaddr_type, key,authenticated, enc_size, ediv, rand);
					...
					const void *param, void *user_data)
{
	...
	if (persistent) {
		...
		store_longtermkey(&key->addr.bdaddr,
...
static void register_mgmt_callbacks(struct mgmt *mgmt, uint16_t index)
{
	...
	mgmt_register(mgmt, MGMT_EV_ADV_MONITOR_REMOVED, index, advmon_removed,
								NULL, NULL);
	mgmt_register(mgmt, MGMT_EV_NEW_LONG_TERM_KEY,index,new_long_term_key_callback,NULL, NULL);
}
